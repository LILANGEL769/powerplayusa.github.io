...
class PartyElectionNomination(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    politician_id = db.Column(db.Integer, db.ForeignKey('politician.id'))
    party_id = db.Column(db.Integer, db.ForeignKey('party.id'))  # This line connects the nomination to a party
    position = db.Column(db.String(50))  # 'chair', 'vice_chair', or 'treasurer'
    votes = db.Column(db.Integer, default=0)

    # Define a relationship to the Politician model
    politician = db.relationship('Politician', backref='nominations')
...
class VoteLog(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    voter_politician_id = db.Column(db.Integer, db.ForeignKey('politician.id'))
    nomination_id = db.Column(db.Integer, db.ForeignKey('party_election_nomination.id'))
...
def vote(nomination_id):
    ...
    if not politician_id:
        return jsonify({'message': 'Please log in to continue. DEBUG: Not Logged In'}), 403

    # Fetch the nomination
    nomination = db.session.get(PartyElectionNomination, nomination_id)
    if not nomination:
        return jsonify({'message': 'Nomination not found. DEBUG: Nomination Not Found'}), 404

    # Check if the politician has already voted for this position in this election
    has_voted = db.session.query(VoteLog).join(PartyElectionNomination).filter(
        VoteLog.voter_politician_id == politician_id,
        PartyElectionNomination.position == nomination.position,
        PartyElectionNomination.party_id == nomination.party_id
    ).first()

    if has_voted:
        return jsonify({'message': 'You have already voted for this position in this election. DEBUG: Already Voted'}), 403

    # Record the vote
    nomination.vote()
    vote_log = VoteLog(voter_politician_id=politician_id, nomination_id=nomination_id)
...
def withdraw_nomination(nomination_id):
    ...
    if nomination.politician_id != session.get('politician_id'):
        return jsonify({'message': 'Not authorized to withdraw this nomination. DEBUG: Not Authorized'}), 403

    # If the user has voted for this position, delete their vote from the VoteLog
    existing_vote = VoteLog.query.filter_by(politician_id=session.get('politician_id'), position=nomination.position).first()
    if existing_vote:
